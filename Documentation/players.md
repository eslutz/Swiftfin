...
Swiftfin offers two player options: **Swiftfin** (VLCKit) and **Native** (AVPlayer). The Swiftfin team recommends using Swiftfin (VLCKit) for optimal compatibility and features, though Native (AVPlayer) is also available for certain cases that benefit from Apple's native capabilities. Apple Vision Pro currently uses Native (AVPlayer) only. All video, audio, and subtitle formats listed are supported for direct playback but may be repackaged based on container support. If transcoding is enabled on your server, any unsupported formats will be converted automatically.
